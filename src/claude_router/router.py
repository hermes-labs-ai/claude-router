...
def _load_pricing(path: Path = PRICING_FILE) -> dict[str, dict[str, Any]]:
    ...
    One catalog backs both this module and the standalone repo-root router, so a price
    is corrected in exactly one place. Every field is checked up front: a malformed or
